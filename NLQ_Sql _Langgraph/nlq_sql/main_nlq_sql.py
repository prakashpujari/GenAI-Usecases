...
SQL:"""
    ...
    sql_output = prompt | llm
    result = sql_output.invoke({"schema": schema_prompt, "question": nlq})
    logging.info(f"LLM OUTPUT | SQL: {result}")
    select_pattern = re.compile(r"(SELECT[\s\S]+?)(;|$)", re.IGNORECASE)
    match = select_pattern.search(str(result))
    if match:
        sql = match.group(1).strip()
        if not sql.endswith(';'):
            sql += ';'
    else:
        sql = str(result).split(';')[0].strip()
